...
var Bandwidth = {
    get: function () {
        var bw = stanza.find(this.xml, NS, 'bandwidth');
        if (!bw.length) {
            return [];
        }
        bw = bw[0];
        return {
            type: bw.getAttribute(this.xml, 'type'),
            bandwidth: bw.getText()
        };
    },
    set: function (value) {
        var bw = stanza.find(this.xml, NS, 'bandwidth');
        if (bw.length) {
            this.xml.removeChild(bw);
        }
        bw = stanza.createElement(NS, 'bandwidth');
        stanza.setAttribute(bw, 'type', value.type);
        stanza.setText(bw, value.bandwidth);
        this.xml.appendChild(bw);
    }
};
