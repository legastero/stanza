...
Disco.prototype = {
    ...
    updateCaps: function () {
        this.caps = {
            node: this.client.config.capsNode || 'https://stanza.io',
            hash: 'sha-1'
            ver: this._generateVerString({
    ...
    _generateVerString: function (info, hash) {
        var S = '',
    ...
    _validateVerString: function (info, hash, check) {
        if (hash === 'sha-1') {
            hash = 'sha1';
        }
