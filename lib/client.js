...
function Client(opts) {
    ...
    this.on('streamFeatures', function (features) {
        ...
        self.featureOrder.forEach(function (name) {
            if (features._extensions[name] && !self.negotiatedFeatures[name]) {
                series.push(self.features[name]);
    ...
    this.features.bind = function (features, cb) {
        console.log('Negotiating Resource Binding');
    ...
    this.features.session = function (features, cb) {
        console.log('Negotiaing Session');
    ...
    this.features.streamManagement = function (features, cb) {
        console.log('Negotiating Stream Management');
        ...
        self.on('stream:management:enabled', 'sm', function (enabled) {
            ...
            self.on('stream:management:ack', 'session', function (ack) {
                self.conn.sm.procAck(ack);
            });
            
            self.on('stream:management:request', 'session', function (request) {
                self.conn.sm.ack();
            });
        ...
        if (self.negotiatedFeatures.streamManagement) {
            cb(null, features);
        } else if (!self.conn.sm.id) {
