...
Client.prototype.connect = function (opts) {
    ...
    if (!self.config.transport) {
        return self.discoverBindings(self.config.server, function (err, endpoints) {
            ...
            } else if (endpoints.websocket.length && self.config.transports.indexOf('websocket') >= 0) {
                self.config.transport = 'websocket';
                self.config.wsURL = endpoints.websocket[0];
                return self.connect();
            } else if (endpoints.bosh.length && self.config.transports.indexOf('bosh') >= 0) {
                console.log('Found BOSH endpoint, but BOSH is not supported');
                self.config.transport = 'bosh';
                return self.disconnect();
            } else {
                console.log('No endpoints found for the requested transports.');
                return self.disconnect();
            }
        });
    }

    self.transports[self.config.transport](self, self.config);
};
