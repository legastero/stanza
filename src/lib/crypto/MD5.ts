...
function fnF(a: number, b: number, c: number, d: number, m: number, k: number, s: number) {
    return (rotl((a + ((b & c) | (~b & d)) + m + k) | 0, s) + b) | 0;
}

function fnG(a: number, b: number, c: number, d: number, m: number, k: number, s: number) {
    return (rotl((a + ((b & d) | (c & ~d)) + m + k) | 0, s) + b) | 0;
}

function fnH(a: number, b: number, c: number, d: number, m: number, k: number, s: number) {
    return (rotl((a + (b ^ c ^ d) + m + k) | 0, s) + b) | 0;
}

function fnI(a: number, b: number, c: number, d: number, m: number, k: number, s: number) {
...
export default class MD5 extends Hash {
    ...
    private _m = new Array(16);

    constructor() {
        super(64, 56, 'le');
    }

    public _update(B: Buffer): void {
        const M = this._m;
        for (let i = 0; i < 16; ++i) {
